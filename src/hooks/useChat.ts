...
// C-13: user?.default_workspace_id instead of user?.tenant_id
...
export function useChat(channelId: string | null) {
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .on(
        ...
        async (payload) => {
          if (payload.new.thread_of) return // replies only in thread panel
          const { data } = await supabase
            .from('messages')
            .select(MSG_SELECT)
            .eq('id', payload.new.id)
            .single()
          if (data) setMessages((prev) => [...prev, data as CommMessage])
        }
      )
      .on(
        'postgres_changes',
  ...
  const send = useCallback(
    async (body: string, attachments: any[] = [], linkPreview: any = null) => {
  ...
  const addReaction = useCallback(
    async (msgId: string, emoji: string) => {
      ...
      const { data: msg } = await supabase
        ...
        .single()
      if (!msg) return
      const r: Record<string, string[]> = msg.reactions || {}
      ...
      await supabase
        .from('messages')
        .update({ reactions: { ...r, [emoji]: updated } })
        .eq('id', msgId)
    },
    [user?.id]
  )
  ...
  const pinMessage = useCallback(
    async (msgId: string) => {
      ...
      await supabase.from('pinned_messages').insert({
        workspace_id: user?.default_workspace_id, // C-13
        channel_id: channelId,
        message_id: msgId,
        pinned_by: user?.id,
      })
